...
while IFS='=' read -r task_key task_value; do
  case "$task_key" in
    DATABASE_URL | REDIS_URL | S3_ENDPOINT | S3_ACCESS_KEY | S3_SECRET_KEY | S3_BUCKET | SAMMYS_COMPOSE_PROJECT | SAMMYS_TEST_NAMESPACE | SAMMYS_POSTGRES_SCHEMA | SAMMYS_REDIS_DB | SAMMYS_REDIS_KEY_PREFIX | SAMMYS_QUEUE_PREFIX | JWT_SECRET | COOKIE_SECRET)
